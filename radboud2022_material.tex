...
\FunctionTok{library}\NormalTok{(tidyverse)}
\CommentTok{\# LinDA is a very new package that we therefore install separately}
\CommentTok{\# directly from Github:}
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\StringTok{"LinDA"} \SpecialCharTok{\%in\%} \FunctionTok{installed.packages}\NormalTok{()[, }\StringTok{"Package"}\NormalTok{]) \{}
\NormalTok{  devtools}\SpecialCharTok{::}\FunctionTok{install\_github}\NormalTok{(}\StringTok{"zhouhj1994/LinDA"}\NormalTok{)}
\NormalTok{\}}
\FunctionTok{library}\NormalTok{(LinDA)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# import our dataset }
\NormalTok{tse }\OtherTok{\textless{}{-}} \FunctionTok{read\_rds}\NormalTok{(}\StringTok{"data/Tengeler2020/tse.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\hypertarget{differential-abundance-analysis-demo}{%
\chapter{Differential abundance analysis demo}\label{differential-abundance-analysis-demo}}

Here, we perform differential abundance analyses using four different methods:
\textbf{Aldex2}, \textbf{ANCOMBC}, \textbf{MaAsLin2} and \textbf{LinDA}.
We will analyse Genus level abundances.

We recommend to first have a look at the \href{https://microbiome.github.io/OMA/differential-abundance.html}{DAA section} of the OMA
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# note that some tools perform prevalence filtering themselves}
\CommentTok{\# so that this step is unncessary depending on the tool used }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# set seed to rule out different results from randomness}
...
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{aldex.clr}\NormalTok{(}
  ...
  \AttributeTok{conds =} \FunctionTok{colData}\NormalTok{(tse)}\SpecialCharTok{$}\NormalTok{patient\_status, }
...
\includegraphics{08-abundance_demo_files/figure-latex/unnamed-chunk-6-1.pdf}
...
\CommentTok{\# install\_github("FrederickHuangLin/ANCOMBC", ref = "develop")}
\end{Highlighting}
\end{Shaded}
...
\section{MaAsLin2}\label{maaslin2}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# first we rarefy data to stick to Nearing et al 2022}
\NormalTok{tse }\OtherTok{\textless{}{-}} \FunctionTok{subsampleCounts}\NormalTok{(}
\NormalTok{  tse,}
  \AttributeTok{abund\_values =} \StringTok{"counts"}\NormalTok{,}
  \AttributeTok{min\_size =} \FunctionTok{min}\NormalTok{(}\FunctionTok{colSums2}\NormalTok{(}\FunctionTok{assay}\NormalTok{(tse))),}
  \AttributeTok{seed =} \FunctionTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{, .Machine}\SpecialCharTok{$}\NormalTok{integer.max),}
  \AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{,}
  \AttributeTok{name =} \StringTok{"subsampled"}\NormalTok{,}
...
\NormalTok{asv }\OtherTok{\textless{}{-}} \FunctionTok{t}\NormalTok{(}\FunctionTok{assay}\NormalTok{(tse, }\StringTok{"subsampled"}\NormalTok{))}
...
\NormalTok{  meta\_data,}
  \CommentTok{\# A folder will be created that is called like the below specified output.}
...
\NormalTok{meta }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(}\FunctionTok{colData}\NormalTok{(tse)) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{select}\NormalTok{(patient\_status)}
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{linda}\NormalTok{(}
\NormalTok{  otu.tab, }
\NormalTok{  meta, }
  \AttributeTok{formula =} \StringTok{\textquotesingle{}\textasciitilde{}patient\_status\textquotesingle{}}\NormalTok{, }
  \AttributeTok{alpha =} \FloatTok{0.05}\NormalTok{, }
  \AttributeTok{prev.cut =} \DecValTok{0}\NormalTok{, }\CommentTok{\# we already filtered }
  \AttributeTok{lib.cut =} \DecValTok{1000}\NormalTok{, }
  \AttributeTok{winsor.quan =} \FloatTok{0.97}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Summary of methods}\label{summary-of-methods}}
...
\NormalTok{summ }\OtherTok{\textless{}{-}} \FunctionTok{full\_join}\NormalTok{(}
    ...
    \FunctionTok{rownames\_to\_column}\NormalTok{(out}\SpecialCharTok{$}\NormalTok{res}\SpecialCharTok{$}\NormalTok{diff\_abn, }\StringTok{"genus"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
      \FunctionTok{select}\NormalTok{(genus, }\AttributeTok{ancombc =}\NormalTok{ patient\_statusControl),}
    \AttributeTok{by =} \StringTok{"genus"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{full\_join}\NormalTok{(}
    \FunctionTok{select}\NormalTok{(fit\_data}\SpecialCharTok{$}\NormalTok{results, }\AttributeTok{genus =}\NormalTok{ feature, }\AttributeTok{maaslin2 =}\NormalTok{ qval) }\SpecialCharTok{\%\textgreater{}\%}
      \FunctionTok{mutate}\NormalTok{(}\AttributeTok{genus =} \FunctionTok{str\_remove}\NormalTok{(genus, }\StringTok{"X"}\NormalTok{)), }
    \AttributeTok{by =} \StringTok{"genus"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{full\_join}\NormalTok{(}
    \FunctionTok{select}\NormalTok{(}
      \FunctionTok{rownames\_to\_column}\NormalTok{(res}\SpecialCharTok{$}\NormalTok{output}\SpecialCharTok{$}\NormalTok{patient\_statusControl, }\StringTok{"genus"}\NormalTok{),}
\NormalTok{      genus, }\AttributeTok{linda =}\NormalTok{ reject),}
      \AttributeTok{by =} \StringTok{"genus"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
...
\FunctionTok{kable}\NormalTok{(}\FunctionTok{head}\NormalTok{(}\FunctionTok{arrange}\NormalTok{(summ, }\FunctionTok{desc}\NormalTok{(score))))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# to plot the highest phylogenetic taxon name }
\NormalTok{taxid\_switch }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(}\FunctionTok{rowData}\NormalTok{(tse)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{rownames\_to\_column}\NormalTok{(}\StringTok{"taxid"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{genus =} \FunctionTok{ifelse}\NormalTok{(Family }\SpecialCharTok{==} \StringTok{""}\NormalTok{, Class, }\FunctionTok{ifelse}\NormalTok{(Genus }\SpecialCharTok{==} \StringTok{""}\NormalTok{, Family, Genus))) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{select}\NormalTok{(taxid, genus) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{column\_to\_rownames}\NormalTok{(}\StringTok{"taxid"}\NormalTok{)}
...
\NormalTok{plot\_data}\SpecialCharTok{$}\NormalTok{patient\_status }\OtherTok{\textless{}{-}} \FunctionTok{colData}\NormalTok{(tse)}\SpecialCharTok{$}\NormalTok{patient\_status}


\CommentTok{\# create a plot for each genus where the score is indicated in the title}
\NormalTok{plots }\OtherTok{\textless{}{-}} \FunctionTok{pmap}\NormalTok{(}
  \FunctionTok{select}\NormalTok{(summ, genus, score), }
  \ControlFlowTok{function}\NormalTok{(genus, score) \{}
\NormalTok{  taxon }\OtherTok{\textless{}{-}}\NormalTok{ taxid\_switch[genus, }\StringTok{"genus"}\NormalTok{]}
  \FunctionTok{ggplot}\NormalTok{(plot\_data, }\FunctionTok{aes\_string}\NormalTok{(}\StringTok{"patient\_status"}\NormalTok{, glue}\SpecialCharTok{::}\FunctionTok{glue}\NormalTok{(}\StringTok{"X\{genus\}"}\NormalTok{))) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{fill =}\NormalTok{ patient\_status), }\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    ...
    \FunctionTok{theme}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"none"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(taxon)}
\NormalTok{\})}

\CommentTok{\# now we can show only those genera that have at least score 4 (3, 2 or 1)}
...
\NormalTok{  robust\_plots[[}\DecValTok{2}\NormalTok{]] }
\end{Highlighting}
\end{Shaded}

\includegraphics{08-abundance_demo_files/figure-latex/unnamed-chunk-14-1.pdf}
...
\CommentTok{\# title how many methods also identified it (we only show first 9 here):}
\NormalTok{linda\_plots }\OtherTok{\textless{}{-}}\NormalTok{ plots[summ}\SpecialCharTok{$}\NormalTok{linda] }
\NormalTok{linda\_plots[[}\DecValTok{1}\NormalTok{]] }\SpecialCharTok{+}\NormalTok{ linda\_plots[[}\DecValTok{2}\NormalTok{]] }\SpecialCharTok{+}
\NormalTok{  linda\_plots[[}\DecValTok{3}\NormalTok{]] }\SpecialCharTok{+}\NormalTok{ linda\_plots[[}\DecValTok{4}\NormalTok{]] }\SpecialCharTok{+}
\NormalTok{  linda\_plots[[}\DecValTok{5}\NormalTok{]] }\SpecialCharTok{+}\NormalTok{ linda\_plots[[}\DecValTok{6}\NormalTok{]] }\SpecialCharTok{+}
\NormalTok{  linda\_plots[[}\DecValTok{7}\NormalTok{]] }\SpecialCharTok{+}\NormalTok{ linda\_plots[[}\DecValTok{8}\NormalTok{]] }\SpecialCharTok{+}
\NormalTok{  linda\_plots[[}\DecValTok{9}\NormalTok{]] }\SpecialCharTok{+}
  \FunctionTok{plot\_layout}\NormalTok{(}\AttributeTok{ncol =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{08-abundance_demo_files/figure-latex/unnamed-chunk-14-2.pdf}
...
\item
  Do ``Differential Abundance'' from the \href{https://microbiome.github.io/OMA/exercises.html}{exercises}.
\end{itemize}
